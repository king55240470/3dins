...
void FileManagerWidget::serializeItem(QDataStream& out, const QStandardItem* item){
    if (!item) {
        out << false; // 标记为空
        return;
    }
    ...
    out << true; // 标记为非空
    out << item->text(); // 序列化文本
    out << item->rowCount(); // 序列化行数
    out << item->columnCount(); // 序列化列数

    // 序列化角色数据
    for (int role = Qt::UserRole; role <= Qt::UserRole + 100; ++role) {
    ...
    for (int row = 0; row < item->rowCount(); ++row) {
        for (int column = 0; column < item->columnCount(); ++column) {
            serializeItem(out, item->child(row, column));
        }
    }
}

QStandardItem* FileManagerWidget::deserializeItem(QDataStream& in){
    bool isNotNull;
    in >> isNotNull;

    if (!isNotNull) {
        return nullptr; // 如果为空，则返回 nullptr
    }
    ...
    for (int row = 0; row < rowCount; ++row) {
        for (int column = 0; column < columnCount; ++column) {
            QStandardItem* child = deserializeItem(in);
