...
void setDataWidget::setPlaneData(pcl::PointXYZRGB searchPoint,pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloudptr){
    ...
    p_rad = new QLineEdit();
    p_rad->setPlaceholderText("0.1");
    p_dis = new QLineEdit();
    p_dis->setPlaceholderText("0.01");
...
void setDataWidget::PlaneBtnClick(){
    ...
    // m_pMainWin->getPointCloudListMgr()->getProductCloudList().push_back(*plane->RANSAC(p_point, p_cloudptr));
}
